        ...
        try:
            ...
            if job.kind in {ToolJobKind.RISK_ALERT.value, ToolJobKind.ALERT_SEND.value}:
                ...
                if not allowed:
                    ...
                    return
            # Policy gate + audit trail: every execution is authorized by
            # ToolPolicyRegistry and recorded in tool_audit_records.
            report = db.get(PsychologicalReport, job.report_id)
            governance = ToolGovernanceService(db)
            governance.require_allowed(job, report)
            audit_record = governance.start_job(job, report)
            job.attempts += 1
            job.updated_at = datetime.utcnow()
            db.add(job)
            db.commit()
            ...
            if audit_record is not None:
                governance.finish(audit_record, ToolStatus.SUCCESS.value, "工具执行成功")
        except Exception as exc:
            if audit_record is not None:
                try:
                    ToolGovernanceService(db).finish(audit_record, ToolStatus.FAILED.value, f"{type(exc).__name__}: {exc}")
                except Exception:
                    logger.exception("Failed to record tool audit failure")
            try:
